...
export class BookTicketComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._aRoute.params.subscribe(data => this.airlinesId = data['airlinesId'])
  ...
  validateDate(c: FormControl) {
    ...
    */
    ...
    const given_date = new Date(c.value);
    return now_date > given_date ? { dateError: { message: "Journey Date cant be a past date" } } : null
  }
  ...
  bookTicket() {
    ...
    */
    ...
    this.ias.bookTicket(this.bookingForm.value).subscribe(
      next => this.successMessage = `You ticket is successfully Booked. Booking id is ${next.id}`,
      err => this.errorMessage = "Booking Failed"
    )
  }
